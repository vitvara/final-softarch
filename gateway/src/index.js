...
function setupHandlers(app) {
    ...
    function getad() {
        var adsout = http.request( // Get a particular video from the metadata microservice.
            {
                host: `advertisement`,
                path: `/ads`,
                method: `GET`,
    ...
    app.get("/", (req, res) => {
        http.request( // Get the list of videos from the metadata microservice.
            ...
            (response) => {
                ...
                response.on("end", () => {
                    getad()
                    if( typeof adsin != 'undefined') {
                        console.log(adsin.name)
                        res.render("video-list", { videos: JSON.parse(data).videos, adname: adsin.name, adurl: adsin.url});
                    }
                    else {
        ...
        http.request( // Get a particular video from the metadata microservice.
            ...
            (response) => {
                ...
                response.on("end", () => {
                    ...
                    if( typeof adsin != 'undefined') {
                        console.log(adsin.name);
                        res.render("play-video", { video , adname: adsin.name, adurl: adsin.url});
                    }
                    else {
    ...
    app.get("/upload", (req, res) => {
        ...
        if( typeof adsin != 'undefined') {
            console.log(adsin.name);
            res.render("upload-video", {adname: adsin.name, adurl: adsin.url});
        }
        else {
    ...
    app.get("/history", (req, res) => {
        http.request( // Gets the viewing history from the history microservice.
            ...
            (response) => {
                ...
                if( typeof adsin != 'undefined') {
                    console.log(adsin.name);
                    res.render("history", { videos: JSON.parse(data).videos , adname: adsin.name, adurl: adsin.url});
